...
func (h *Handler) GetProcessedImage(c *ginext.Context) {
	...
	if err != nil {
		...
		// 	return
		// } else if errors.Is(err, images.ErrImageNotReady) {
		// 	zlog.Logger.Warn().Err(err).Msg("image not ready yet")
		// 	handlers.Fail(c.Writer, http.StatusServiceUnavailable, fmt.Errorf("image not ready yet"))
		// 	return
		// }
